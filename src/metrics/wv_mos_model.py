...
class Wav2Vec2MOS(nn.Module):
    def __init__(self, path, freeze=True, cuda=True):
        ...
        self.dense = nn.Sequential(nn.Linear(768, 128), nn.ReLU(), nn.Dropout(0.1), nn.Linear(128, 1))
    ...
    def calculate_batch(self, signal):
        x = self.processor(signal, return_tensors="pt", padding=True, sampling_rate=16000).input_values[0]
